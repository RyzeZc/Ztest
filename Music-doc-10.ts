...
function initializeMusicPlayer(): void {
    ...
    > {
        ...
        console.log(
            '[MusicPlayer] YouTube playlist loaded:',
            playlistId,
            tracks.length,
            'tracks'
        );
    ...
    function findRelevantYouTubePlaylist(
    query: string,
    playlists: Array<{
        playlistId: string;
        title: string;
    ...
    scoredPlaylists.sort(
        (a, b) =>
            b.score - a.score
    );

    return (
        scoredPlaylists[0]?.score > 0
            ? scoredPlaylists[0].playlist
            : null
    );
}
...
async function findYouTubePlaylistWithMatchingTrack(
    ...
    for (
        ...
        if (!response.ok) {
            ...
            console.log(
                '[MusicPlayer] Unable to load playlist:',
                playlist.playlistId
            );
        ...
        if (matchingTrack) {
            ...
            console.log(
                '[MusicPlayer] Matching playlist found:',
                playlist
            );

            console.log(
                '[MusicPlayer] Matching track:',
                matchingTrack
            );
...
async function searchYouTubePlaylistsInternal(
    ...
    try {
        ...
        if (
            ...
            console.log(
                '[MusicPlayer] No YouTube playlists found.'
            );
        ...
        const playlistMatch =
            await findYouTubePlaylistWithMatchingTrack(
                query,
                playlists
            );
        ...
        if (!playlistMatch) {
            ...
            console.log(
                '[MusicPlayer] No relevant YouTube playlist found.'
            );
        ...
        const tracks =
            playlistMatch.tracks;

        console.log(
            '[MusicPlayer] Selected YouTube playlist:',
            testPlaylist
        );

        console.log(
            '[MusicPlayer] Selected playlist tracks:',
            tracks
        );
